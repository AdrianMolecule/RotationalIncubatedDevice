...
class Config {
   public:
    ...
    static inline const char* FIRMWARE_VERSION = "TR-Tube Rotator 1.3";
    static inline const char* DNS = "biotr";  // 192.168.0.191
    ...
    static void initializeHardcodedFields(std::vector<Field>& fields) {
        ...
        fields.emplace_back(getNextIdStr(), "info", "string", "", "logs info for the device", true /*RO*/, true /*shown */, false /*not persisted */);  // MANDATORY
    };
        //
        static inline int idCounter = 0;
        static const int initialSpeakerPin = NOT_PRESENT_INT;
        //
        static void initialize(std::vector<Field> & fields) {
            ...
            fields.emplace_back(getNextIdStr(), "stepperOn", "bool", "1", "Turns stepper on off ", false, true, false);
            ...
            fields.emplace_back(getNextIdStr(), "SpeakerPin", "uint8_t", String(initialSpeakerPin), "SpeakerPin");
